...
class CopyEngine:
    ...
    def __init__(self, msg_queue: queue.Queue, settings: CopySettings):
        ...
        # Bolt: Pre-calculate EMA complement and hasher factory to save work in hot methods
        self._ema_complement = 1.0 - self._ema_alpha
        self._hasher_factory = self._get_hasher_factory(settings.hash_algorithm)
    ...
    def _get_hasher_factory(self, algo: str):
        """Bolt: Returns a factory function for the chosen hash algorithm."""
        # Using hashlib.blake2b() directly is ~3x faster than hashlib.new('blake2b')
    ...
    def _scan_source(self):
        ...
        dirs_scanned = 0
        total_bytes = 0
        # Bolt: Store (absolute, relative) tuples in stack to avoid redundant slicing
        stack = [(root, "")]
        ...
        # Bolt: operator.itemgetter is ~25-30% faster than lambda for sorting
    ...
    def _send_stats(self, current_file_bytes: int = 0, current_file_total: int = 0):
        ...
        if self._ema_rate <= 0:
            ...
            # Bolt: Use pre-calculated complement to save a subtraction
...
# Bolt: Constant units tuple avoids creation on every call to fmt_bytes
